...
public class EnemyAI : MonoBehaviour 
{
	...
	public int Score;
	public bool NotDamagable = false;
	public bool NotAttackable = false;
	public bool NotBouncable = false;
	...
	void Update()
	{
		if (!GameObject.FindGameObjectWithTag ("LevelController").GetComponent<LevelController> ().Pause) 
		{
			...
			if (attackTrigger && (attackTrigger.collided))
			{
				...
				if (attackTrigger.hitObject.tag == "Player" && attackTrigger.hitObject.GetComponent<PlayerMove>().IsRolling)
				{
					NotAttack = true;
				}
				if (!NotAttack)
				{
					if (!NotAttackable)
					{
						if (!attackTrigger.hitObject.GetComponent<PlayerMove>().AntiAttack)
						{
				...
				else
				{
					...
					attackTrigger.hitObject.GetComponent<PlayerMove>().RollAttack(gameObject);
				}
			}
	...
	{	
		...
		if (HasThorn)
		{
			dealDamage.Attack (GameObject.FindGameObjectWithTag("Player").gameObject, attackDmg, pushHeight , pushForce );
		}
		if(!playerMove)
